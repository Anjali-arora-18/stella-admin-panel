...
const visibleColumns = computed(() => {
  return defineVaDataTableColumns(
...
const filteredItems = computed(() => {
  ...
  return items.value
    ...
    .filter(
      item =>
        item.wCode?.toLowerCase().includes(query) ||
...
<div class="relative" ref="columnsMenuWrapper">
  ...
  <div class="editable-field relative group">
    <input
      ...
      class="editable-input"
      autofocus
      @blur="(rowData.editDescription = false), emits('updateCategory', { description: rowData.description, _id: rowData._id })"
    />
