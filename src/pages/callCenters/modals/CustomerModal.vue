...
async function addAddress() {
  ...
  // Only verify availability when adding a new address AND a street result was chosen
  if (editAddress.value === -1 && addressSet.value) {
    try {
...
async function stellaUpsertFromForm(
  ...
  if (hit) {
    return axios.patch(
      `${import.meta.env.VITE_API_BASE_URL}/customers/${hit._id || hit.id}`,
      { ...common, id: hit._id || hit.id },
    );
  } else {
...
async function winmaxCreateOrUpdate(base: any, outletId: string, selected?: any) {
  ...
  if (hasWmId) {
    ...
    const res = await axios.post(
      `${import.meta.env.VITE_API_BASE_URL}/winmax/entities`,
      wmPayload,
      { params: { outletId } },
    )
    let meta =
