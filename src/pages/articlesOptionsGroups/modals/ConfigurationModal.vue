...
const getArticlesGroups = async () => {
  ...
  await axios.get(`${url}/menuItemvoById/${selectedItems.value.menuItemId}`).then((response) => {
    articlesGroup.value = response.data.articlesOptionsGroups.map((group) => {
      return {
        ...group,
...
<template>
  ...
  >
    <template #header>
      <h1 class="va-h6 mb-2">Configuration</h1>
    </template>
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 max-h-[70vh] overflow-y-auto px-2">
      <div class="row align-items-center mb-2">
        <div class="border rounded shadow-sm p-2">
          <h3 class="text-sm font-semibold mb-2">Categories / SubCategories</h3>
      ...
      <div class="row align-items-center mb-2">
        <div class="border rounded shadow-sm p-2">
          <h3 class="text-sm font-semibold mb-2">Article Options Group</h3>
          <table class="w-full border-collapse border border-gray-200 text-sm">
            <tbody v-if="!isArticleGroupLoading">
              <tr v-for="articleGroup in articlesGroup" :key="'articlesGroups-' + i" class="border-b hover:bg-gray-50">
                <td class="p-2 cursor-pointer">
                  <div>
      ...
      <div class="row align-items-center mb-2">
        <div class="border rounded shadow-sm p-2">
          <h3 class="text-sm font-semibold mb-2">Articles Options</h3>
          <table class="w-full border-collapse border border-gray-200 text-sm">
            <tbody>
              <tr
