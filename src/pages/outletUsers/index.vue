...
import StellaUsersTable from '@/pages/outletUsers/widgets/UsersTable.vue'
...
const getStellaUsersCount = (outletId) => {
  ...
  axios.get(`${url}/users/count?search=${searchQuery.value}&outletId=${outletId}`).then((response) => {
...
watch(
  ...
  (outletId) => {
    if (outletId) {
      getStellaUsers(outletId)
      getStellaUsersCount(outletId)
    }
  },
  { immediate: true },
)
